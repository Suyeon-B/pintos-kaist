...
tid_t
process_create_initd (const char *file_name) { //실행파일의 이름을 가져온다. (커멘드라인???)
	char *fn_copy;
	tid_t tid;
 ...
 */
int
process_exec (void *f_name) { //프로세스 실행 - 실행하려는 바이너리 파일 이름을 가져옴
...
static bool
load (const char *file_name, struct intr_frame *if_) {// 사용자 스택 유형, 함수의 시작진입 점등을 포함한다.
